...
function AppMenu() {
  ...
  return (
    <div className="pt-32 pb-12 flex flex-col justify-between items-center bg-stone-800 border-r-[0.5px] border-stone-600 shadow-lg ">
        ...
        <ul className=" text-sm font-semibold flex flex-col gap-4">
          {tabs.map((tab, index) => {
            ...
            return (
              <li key={index}>
                <a
                  className={`relative py-2 pl-8 flex items-center gap-3 cursor-pointer capitalize transition-all duration-200 hover:text-main-500 ${
                    active === tab.name ? "text-main-500" : "text-stone-300"
                  }`}
                  onClick={() => handleClick(tab)}
                >
                  <Icon className=" text-2xl" /> {tab.name}
                  {
                    <div
                      className={`w-1 h-full absolute left-0 bg-main-500 transition-all duration-200 origin-left ${
...
function AddMarathonModal({ isOpen, setIsOpen }) {
  // QueryClient to invalidate queries after creation
  const queryClient = useQueryClient();
  //  useMutation to add the new row to database
